...
class OutboxService
{
    ...
    public function enqueuePermissionChanged(int $roleId, array $permissionIds, string $action): EventOutbox
    {
        return $this->enqueue(config('events.channels.permission_changed'), [
            'role_id' => $roleId,
        ...
        foreach ($rows as $row) {
            try {
                ...
                if ($row->attempts >= $maxAttempts) {
                    ...
                    NotificationService::notifyOutboxFailure($row, $exception->getMessage());
                }
            }
        }
