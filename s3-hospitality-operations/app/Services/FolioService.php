...
class FolioService
{
    ...
    public function addCharge(Folio $folio, string $description, float $amount, string $category = 'room'): FolioLine
    {
        ...
        $revenueAccount = match ($category) {
            ...
            'room' => $accounts['room_revenue'],
            default => $accounts['service_charge_revenue'],
        };
    ...
    public function recordPayment(Folio $folio, array $data, int $cashierId): FolioPayment
    {
        ...
        if ($shiftId <= 0 && $this->paymentMethodRequiresShift($paymentMethod)) {
            throw new InvalidArgumentException('cashier_shift_id is required for cash payments.');
        }
        ...
        return DB::transaction(function () use ($folio, $amount, $paymentMethod, $cashierId, $shiftId, $idempotencyKey, $cashAccount, $balance) {
            ...
            $payment = FolioPayment::query()->create([
                ...
                'cashier_shift_id' => $shiftId > 0 ? $shiftId : null,
                'paid_at' => now(),
    ...
    private function paymentMethodRequiresShift(string $paymentMethod): bool
    {
        return in_array($paymentMethod, ['cash'], true);
    }
}
